...
function buildPickerSystemPrompt(
  ...
  const lines: string[] = [
    ...
    'ESPECIALLY for gratitude/apology/confrontational/emotional messages: Match source word count ±1-2 words only. Do not add "but", "because", "I feel", "it\'s not", "we must change", or meta-commentary.',
    'In conflict especially: Do NOT add elaborative clauses even if emotional intensity is high. "We can\'t keep doing this" stays concise—do NOT expand to "I can\'t take it anymore, we must change" (adds demands/solutions not in source).',
    ...
    'CRITICAL CONFLICT RULE: In accusatory/confrontational messages, do NOT add implied consequences, refusal, threats, or severity not in source.',
    'Examples of what NOT to invent:',
    '- "Tôi muốn cảm ơn anh/chị" (want to thank you—NO object) → "I want to thank you", NOT "I want to thank you for what you\'ve done" (invented object)',
    '- "Cảm ơn" (thanks—general) → "Thank you", NOT "Thank you for your help" (unspecified help)',
    '- "Đó là quyết định đúng" (right decision—no subject) → "That\'s the right decision", NOT "You made the right choice" (invented subject)',
    '- "Anh/chị không đơn độc" (not alone—no agents) → "You\'re not alone", NOT "You have people who care" (invented supporters)',
    'CONFLICT-SPECIFIC: Do NOT add these in accusatory contexts:',
    '- "Em đã cảnh báo anh/chị rồi" (warned you—no consequence stated) → "I warned you", NOT "I told you this would happen" (invented consequence)',
    '- "Anh/chị không hiểu tôi" (don\'t understand—ambiguous intent) → "You don\'t understand me", NOT "You refuse to understand" (adds malicious intent)',
    '- "Đó là lần cuối cùng" (last time—no threat) → "That\'s the last time", NOT "This is your last chance" (invents threat/ultimatum)',
    '- "Anh/chị không xứng đáng" (not worthy—no object) → "You\'re not worthy", NOT "You don\'t deserve it" (invented deserving of what?)',
    'FAREWELL-SPECIFIC: Do NOT add these in goodbye/farewell contexts:',
    '- "Em sẽ nhớ khoảnh khắc này" (remember this moment—no "with you" specified) → "I\'ll remember this moment", NOT "I\'ll remember this moment with you" (invents togetherness not stated)',
    '- "Cảm ơn vì tất cả" (thank you for everything—no actions specified) → "Thank you for everything", NOT "Thank you for what you\'ve done" (invents specific actions)',
    '- "Tạm biệt" (goodbye—ambiguous finality) → "Goodbye", NOT "See you later" (invents future meeting when source could mean final goodbye)',
    ...
    'CASUAL/SIMPLE STATEMENTS: For brief, straightforward casual messages, stay LITERAL and BRIEF. Do NOT elaborate with implications, expectations, or desires not in source.',
    'Examples of what NOT to invent:',
    '- "That sounds fun" → "That sounds fun" ONLY, NOT "That sounds fun, I hope we can do it" (invents expectation)',
    '- "I\'m tired" → "I\'m tired", NOT "I\'m tired and need a break" (invents implicit need/desire)',
    '- "Cool" → "Cool", NOT "That\'s cool and I like it" (elaborates simple agreement)',
    '- "Okay" → "Okay", NOT "Okay, I understand and agree" (over-specifies simple acknowledgment)',
    'RULE: Simple statements without emotional markers (no !, no emoji, no intensifiers) should remain simple and literal. Preserve the brevity and directness of the source.',
    '',
  ...
  Use context, previous message topic, and sentence structure to disambiguate. When in doubt, pick the meaning that fits the conversation flow.`
    ...
    `- ${srcIsVietnamese ? 'CONFIDENCE SCORES (CRITICAL FOR AMBIGUOUS VERBS AND COLLOQUIAL TERMS): For Vietnamese input, include "confidenceScore" (0-100) for EACH option. Score how confident you are that this option correctly interprets any ambiguous verbs (được, để, tới, có, ghê, hay, mà, vì, etc.) AND any colloquial/affectionate terms detected. Higher score = more certain this interpretation fits the context. Lower score = multiple plausible interpretations exist. This score determines whether the picker is shown to the user (high confidence=auto-select, low confidence=show picker).' : 'CONFIDENCE SCORES: English input should set confidenceScore to null.'}`,
